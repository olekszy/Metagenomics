...
do
	...
	samtools sort -O bam -o sorted"$sam".bam "$sam".bam
done
...
do
	...
	java -jar /home/fagi/picard.jar MarkDuplicates I=$sam.bam O=Duplicate$sam.bam M=Duplicates$sam_metrics.txt REMOVE_DUPLICATES=true
done
# samtools faidx / 
...
for z in Duplicate*.bam
do
	sam=$(echo "$z" | rev |cut -d"." -f2- | rev )
	echo  "\e[32m Basecalling $sam \e[0m"	
	bcftools mpileup -Ou -f /home/fagi/grch38/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna $sam.bam | bcftools call -vmO z -o $sam.bcf #Variant calling z bcftools
	#samtools mpileup -B -f /home/fagi/grch38/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna $sam.bam | java -jar /home/fagi/VarScan.v2.3.9.jar pileup2cns --min-coverage 2 --min-var-freq 0.8 --p-value 0.005 --variants --output-vcf > variants.vcf
